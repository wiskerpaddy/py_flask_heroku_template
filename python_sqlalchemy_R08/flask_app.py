...
# CAUTION!! this software is 'test'code. if you use this for work , 'SECRET_KEY' must be created outside of this code.
...
def __init__(self, name, travel_costs, date,pjcode):
   self.name = name
...
def check_input_value(req_form_travel,req_form_date,req_form_name,req_form_pjcode):
    ...
    if input_name == none_name or input_travelcosts == none_travelcosts or input_date == none_date or input_pjcode == none_pjcode:
        ...
        #flash('不正な値が書き込まれました。入力内容を確認してください。')
        message = False
    else:
        #message = '正常に書き込みました。'
        #flash('正常に書き込みました。')
...
def select_sql():
    ...
    return render_template('menu.html', message = message, seisandb = seisandb)
...
def result():
    if request.method == 'POST':
        ...
        if message == True:
            ...
            flash('不正な値が書き込まれました。入力内容を確認してください。','error')
