...
def result():
    ...
    seisandb_up = SeisanDB(name=input_name,travel_costs=input_travelcosts,date=input_date,pjcode=input_pjcode)
    db.session.add(seisandb_up)
    db.session.commit()

    return render_template('result.html', message = message, name = input_name, travel_costs = input_travelcosts, date = input_date, pjcode = input_pjcode)
